...
function Replace-LineBytes([string]$SourcePath, [string]$DestinationPath, [int]$TargetLine, [string]$Replacement) {
    ...
    if ($lineEnd -lt 0 -or $lineNumber -ne $TargetLine) {
        throw "Unable to locate $SourcePath line $TargetLine."
    }
...
function New-ServerCompatibilityCopy([string]$SourcePath) {
    ...
    Write-Host '[legacy] Server.ci.cpp: normalized one non-semantic pragma region label without transcoding the file.'
    return $destination
}
...
function New-CompatibilityProjectCopy([string]$ProjectPath, [string]$ProjectName) {
    ...
    if ($compileNodes.Count -eq 0) {
        throw "No ClCompile items found in $ProjectName."
    }

    if ($ProjectName -eq 'TMSrv') {
        $serverNode = $compileNodes | Where-Object { $_.GetAttribute('Include') -eq 'Server.cpp' } | Select-Object -First 1
        if ($null -eq $serverNode) {
            throw 'Server.cpp compile item was not found in TMSrv.'
        }
        $serverNode.SetAttribute('Include', 'Server.ci.cpp')
    }
...
if (-not (Test-Path (Join-Path $mysqlIncludeDir 'mysql.h'))) {
    throw "Vendored MySQL headers not found: $mysqlIncludeDir"
}
...
if ($Project -eq 'TMSrv' -or $Project -eq 'All') {
    $projects += @{ Name = 'TMSrv'; Path = (Join-Path $codeRoot 'TMSrv/TMSrv.vcxproj') }
}
if ($Project -eq 'DBSrv' -or $Project -eq 'All') {
    $projects += @{ Name = 'DBSrv'; Path = (Join-Path $codeRoot 'DBSrv/DBSrv.vcxproj') }
}
...
if ($CompileOnly) {
    Write-Host 'Legacy compile-only gate completed; link intentionally skipped.'
} else {
